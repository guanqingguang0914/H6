...
int httpd::_read(int fd, iobuffer *iobuf, void *buffer, size_t len, int timeout) {
  ...
  while ( (copied < (int)len) ) {
    ...
    //memcpy(buffer+copied, iobuf->buffer+IO_BUFFER-iobuf->level, i); //there is compile error here, chenrui
    int *buffer2 = (int *)buffer;
    memcpy(buffer2+copied, iobuf->buffer+IO_BUFFER-iobuf->level, i);
...
void httpd::send_file(int fd, char *parameter) {
  ...
  /* try to open that file */
   // if ( (lfd = fopen(buffer, O_RDONLY)) < 0 ) {
  if ( (lfd = open(buffer, O_RDONLY)) < 0 ) {
    DBG("file %s not accessible\n", buffer);
    send_error(fd, 404, "Could not open file");
    LOGE(">>>>>send_file>>>3 ");
    LOGE("trying to serve file \"%s\", extension: \"%s\" mime: \"%s\"\n", parameter, extension, mimetype);
    LOGE("trying to serve file \"%s\"\n", buffer);
    return;
  }
  DBG("opened file: %s\n", buffer);
...
void httpd::input_cmd(in_cmd_type cmd, float value, char* res_str)
{
    ...
    case IN_CMD_WALK_AUTO_BALANCE:
        ...
        break;
    case IN_CMD_WALK_AUTO_POSE:
        Walking::GetInstance()->POSE_ENABLE = value;
        if(Walking::GetInstance()->POSE_ENABLE) strcpy(res_str, "ON");
        else strcpy(res_str, "OFF");
        break;
    ...
    case IN_CMD_WALK_B_ANKLE_ROLL_GAIN:
        ...
        break;
    case IN_CMD_WALK_P_KNEE_GAIN:
        Walking::GetInstance()->POSE_KNEE_GAIN += value;
        sprintf(res_str, "%.2f", Walking::GetInstance()->POSE_KNEE_GAIN);
        break;
    case IN_CMD_WALK_P_ANKLE_PITCH_GAIN:
        Walking::GetInstance()->POSE_ANKLE_PITCH_GAIN += value;
        sprintf(res_str, "%.2f", Walking::GetInstance()->POSE_ANKLE_PITCH_GAIN);
        break;
    case IN_CMD_WALK_P_HIP_ROLL_GAIN:
        Walking::GetInstance()->POSE_HIP_ROLL_GAIN += value;
        sprintf(res_str, "%.2f", Walking::GetInstance()->POSE_HIP_ROLL_GAIN);
        break;
    case IN_CMD_WALK_P_ANKLE_ROLL_GAIN:
        Walking::GetInstance()->POSE_ANKLE_ROLL_GAIN += value;
        sprintf(res_str, "%.2f", Walking::GetInstance()->POSE_ANKLE_ROLL_GAIN);
        break;
...
void* httpd::client_thread( void *arg ) {
  ...
  if ( (cnt = _readline(lcfd.fd, &iobuf, buffer, sizeof(buffer)-1, 5)) == -1 ) {
    ...
    LOGE(">>>enter into client_thread>>error1>");
    close(lcfd.fd);
    return NULL;
  }
  ...
  if ( strstr(buffer, "GET /?action=snapshot") != NULL ) {
    ...
    if ( (pb = strstr(buffer, "GET /?action=command")) == NULL ) {
      ...
      LOGE(">>>HTTP request seems to be malformed>");
    ...
    LOGE(">>>try to serve a file");
    req.type = A_FILE;

    if ( (pb = strstr(buffer, "GET /")) == NULL ) {
      DBG("HTTP request seems to be malformed\n");
      LOGE(">>>HTTP request seems to be malformed 2>");
      send_error(lcfd.fd, 400, "Malformed HTTP request");
      close(lcfd.fd);
      return NULL;
    }
...
void* httpd::server_thread( void *arg ) {
  ...
  if ( server->sd < 0 ) {
    fprintf(stderr, "socket failed\n");  //create socket failed, so it will crash
    exit(EXIT_FAILURE);
  }
  ...
  addr.sin_port = server->conf.port; 
  ...
  while ( 1  ) {
    ...
    if( pthread_create(&client, NULL, &client_thread, pcfd) != 0 ) {
      ...
      LOGE(">>>>>could not launch another client thread>>>");
      close(pcfd->fd);
      free(pcfd);
      continue;
    }
