...
#define  LOG_TAG    "mjpg_streamer"
#define  LOGI(...)  __android_log_print(ANDROID_LOG_INFO, LOG_TAG, __VA_ARGS__)
#define LOGE(...)  __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, __VA_ARGS__)
#define LOGI(...)  __android_log_print(ANDROID_LOG_INFO, LOG_TAG, __VA_ARGS__)

#define WWW_FOLDER          "/mnt/sdcard/www/"
//#define WWW_FOLDER          "/mnt/Abilix/walktunner/www/"
...
int mjpg_streamer::send_image(Image* img)
{
	if(httpd::ClientRequest == true)
	{
		...
		if(img->m_PixelSize == Image::YUV_PIXEL_SIZE)
			global.size = 80; //jpeg_utils::compress_yuyv_to_jpeg(input_yuv, global.buf, input_yuv->m_ImageSize, 80);  //chenrui removed
		else if(img->m_PixelSize == Image::RGB_PIXEL_SIZE)
			global.size = 80; //jpeg_utils::compress_rgb_to_jpeg(input_rgb, global.buf, input_rgb->m_ImageSize, 80); //chenrui removed
